import { User } from "../entities/user";
import { Bean } from "../context/context";
import { UserRepository, idUserRepository } from "../repositories/user-repo";
import md5 from 'md5';
import jwt from 'jsonwebtoken';
import { UnauthorizedException } from "../exceptions/unauthorized-exception";

export const idAuthService = 'app-services-AuthService';

export class AuthService extends Bean {

    private key: string;

    async __init() {
        this.key = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`;
    }

    private userRepo: UserRepository;

    async __initUserRepo() {
        if (!this.userRepo) {
            this.userRepo = await this.__ctx.getBean(idUserRepository, UserRepository) as UserRepository;
        }
    }

    async login(email: string, password: string): Promise<string> {
        const encodedPassword: string = md5(password);
        await this.__initUserRepo();
        const user  = await this.userRepo.findByEmailAndPassword(email, encodedPassword);
        const iat = new Date().getTime();
        const claims = {
            sub: email,
            iat: iat,
            exp: iat + 3600000,
            roles: user.rolesList,
        };
        const token = jwt.sign(claims, this.key, { algorithm: 'RS256' });
        return token;
    }

    rolesOverlap(roles1: string[], roles2: string[]): boolean {
        for (let i = 0; i < roles1.length; i++) {
            if (roles2.includes(roles1[i])) {
                return true;
            }
        }
        return false;
    }

    async hasRights(event: any, roles: string[]): Promise<User> {
        await this.__initUserRepo();
        const header: string = event?.headers?.Authentication;
        if (!header?.startsWith('Bearer ')) {
            throw new UnauthorizedException('missing Authentication header');
        }
        const token = header.substring('Bearer '.length);
        let claims;
        try {
            claims = jwt.verify(token, this.key, { algorithm: 'RS256' });
        } catch(err) {
            console.log('Error:', err['message'] && JSON.stringify(err));
            throw new UnauthorizedException('failed to verify JWT');
        }
        const user = await this.userRepo.findByEmail(claims['sub']);
        if (!user) {
            throw new UnauthorizedException('user does not exist');
        }
        if (!this.rolesOverlap(roles, claims.roles)) {
            throw new UnauthorizedException('user lacks rights to perform this operation');
        }
        return user;
    }
}